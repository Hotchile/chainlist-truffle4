// load contract to be articleDescription
...
// buy an article that does not exist
  ...
  it("should throw exception if you try to buy an article that has already been sold", function(){
    return ChainList.deployed().then(function(instance){
      chainListInstance = instance;
    ...
    .catch(function(error){
      ...
      assert.equal(data[0].toNumber(), 1, "artilce id must be one");
      assert.equal(data[1], seller, "seller must be " + seller);
